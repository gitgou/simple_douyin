...
func (s *RedisServiceImpl) Set(ctx context.Context, req *redisdemo.SetRequest) (resp *redisdemo.SetResponse, err error) {
	...
	if err != nil {
		klog.Errorf("Set Key error %s, Key:%s, Value %d ", err.Error(), req.Key, req.Value)
...
func (s *RedisServiceImpl) GetIncreId(ctx context.Context, req *redisdemo.GetIncreIdRequest) (resp *redisdemo.GetIncreIdResponse, err error) {
	...
	if err != nil {
		klog.Errorf("Incre Key error:%s, Key:%s ", err.Error(), req.Key)
		resp.BaseResp = &userdemo.BaseResp{StatusCode: 1, StatusMsg: err.Error()}
		return resp, err
	}
...
func (s *RedisServiceImpl) ZSetGetMember(ctx context.Context, req *redisdemo.ZSETGetMemberRequest) (resp *redisdemo.ZSETGetMemberResponse, err error) {
	...
	if err != nil {
		klog.Errorf("Incre Key error %s, %s", err.Error(), req.Key)
		return resp, err
	}
...
func (s *RedisServiceImpl) GetUserInfo(ctx context.Context, req *redisdemo.GetUserInfoRequest) (resp *redisdemo.GetUserInfoResponse, err error) {
	...
	if err != nil {
		klog.Error("Get Zset Score error ", err.Error(), ", key: ", constants.RedisZSetKeyFollow)
	} else {
	...
	if err != nil {
		klog.Error("Get Zset Score error ", err.Error(), ", key: ", constants.RedisZSetKeyFollower)
	} else {
	...
	if err != nil {
		klog.Error("Get Zset Score error ", err.Error(), ", key: ", constants.RedisZSetKeyFavorite)
	} else {
	...
	if err != nil {
		klog.Error("Get Zset Score error ", err.Error(), ", key: ", constants.RedisZSetKeyFavorited)
	} else {
	...
	if err != nil {
		klog.Error("Get Zset Score error ", err.Error(), ", key: ", constants.RedisZSetKeyVideo)
	} else {
